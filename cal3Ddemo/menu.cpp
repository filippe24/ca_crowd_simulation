...
void Menu::calculateMotionBlend(int x, int y)
{
  ...
  if((y >= MENUITEM_Y[Modelloader::STATE_MOTION]) && (y < MENUITEM_Y[Modelloader::STATE_MOTION] + MENUITEM_HEIGHT[Modelloader::STATE_MOTION]))
  {
...
bool Menu::onMouseButtonDown(int button, int x, int y)
{
  ...
  // handle 'idle' button
  if(menuItem == Modelloader::STATE_IDLE)
  {
    theDemo.getModel()->setState(Modelloader::STATE_IDLE, 0.3f);
    return true;
  }

  // handle 'fancy' button
  if(menuItem == Modelloader::STATE_FANCY)
  {
    theDemo.getModel()->setState(Modelloader::STATE_FANCY, 0.3f);
    return true;
  }

  // handle 'motion' button/controller
  if(menuItem == Modelloader::STATE_MOTION)
  {
    theDemo.getModel()->setState(Modelloader::STATE_MOTION, 0.3f);
...
void Menu::onRender()
{
  ...
  if(state == Modelloader::STATE_MOTION)
  {
